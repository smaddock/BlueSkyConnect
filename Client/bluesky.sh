...
function reKey {
  ...
  # upload pubkey
  installResult=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "newpub=$pubKey" https://$blueskyServer/cgi-bin/collector.php`
  curlExit=$?
  ...
  uploadResult=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=register --data-urlencode "hostName=$hostName" https://$blueskyServer/cgi-bin/collector.php`
  curlExit=$?
...
port=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=port https://$blueskyServer/cgi-bin/collector.php`
curlExit=$?
...
defaultUser=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=user https://$blueskyServer/cgi-bin/collector.php`
if [ "$defaultUser" != "" ]; then
...
connStat=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=status https://$blueskyServer/cgi-bin/collector.php`
if [ "$connStat" != "OK" ]; then
  ...
  restartConnection
  sleep 5
  connStatRetry=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=status https://$blueskyServer/cgi-bin/collector.php`
  if [ "$connStatRetry" != "OK" ]; then
    ...
    restartConnection
    sleep 5
    connStatLastTry=`curl $curlProxy -s -S -m 60 -1 --retry 4 -X POST --data-urlencode "serialNum=$serialNum" -d actionStep=status https://$blueskyServer/cgi-bin/collector.php`
